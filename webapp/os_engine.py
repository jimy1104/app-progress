...
def _encabeza(texto: str, pos: int) -> bool:
    """¿El título empieza la línea? Se admite un numeral delante («1. TÉRMINOS…»)."""
    i = pos
    while i > 0 and texto[i - 1].isdigit():
        i -= 1
    return i == 0 or texto[i - 1] == "|"
...
DOCS = {
    ...
    "comprobante_pago": dict(
        ...
        requiere=[], excluye=["CONLAFACTURA", "SUFACTURACOPIA"], hojas=1),
    "validez_cpe": dict(
        etiqueta="Consulta de validez del comprobante",
        anclas=["CONSULTAVALIDEZDELCOMPROBANTE"], requiere=[], excluye=[], hojas=1),
    "informe": dict(
        etiqueta="Informe",
        anclas=["INFORMEN", "INFORMEDELSERVICIO"], requiere=[],
        excluye=["REFERENCIAINFORMEN", "ELINFORMEN", "SEGUNINFORMEN"], hojas=None),
    "memorando": dict(
        etiqueta="Memorando",
        anclas=["MEMORANDON", "MEMORANDUMN", "OFICION"], requiere=[],
        excluye=["REFERENCIAMEMORANDON", "ELMEMORANDON", "SEGUNMEMORANDON"], hojas=None),
    "anexo": dict(
        etiqueta="Anexo / Declaración jurada",
        anclas=["ANEXO0", "ANEXON", "DECLARACIONJURADA"], requiere=[], excluye=[], hojas=1),
    "cotizacion": dict(
        etiqueta="Cotización / Cuadro comparativo",
        anclas=["COTIZACIONN", "CUADROCOMPARATIVO", "PROFORMAN"], requiere=[], excluye=[],
        inicio_linea=True, hojas=None),
}
...
def _tipo_de_pagina(pg):
    ...
    for tipo, d in DOCS.items():
        ...
        for ancla in d["anclas"]:
            ...
            # renglón eso no pasa.
            for texto in [texto_cab] + lineas_cab:
                ...
                if d.get("con_numero") and not re.match(r"[SNO]{0,3}\d", texto[pos + len(ancla): pos + len(ancla) + 10]):
                    continue
                if not mejor or score > mejor[1]:
                    mejor = (tipo, score)
    if mejor:
        return mejor
    if _es_memo(pg):
        return ("memorando", 0.75)
    return None
...
DEBILES = {"anexo", "informe", "memorando", "cotizacion"}


def _filtra_continuaciones(doc, inicios):
    """Una hoja que sigue a un TDR y lleva su membrete es CONTINUACIÓN del TDR,
    aunque mencione «declaración jurada» o «informe» en una cláusula."""
    out = []
    for pag, tipo, conf in inicios:
        if out and out[-1][1] == "tdr" and tipo in DEBILES:
            if _parecido(_compacta(doc.pages[pag - 1].text), "TERMINOSDEREFERENCIA", 0.90):
                continue
        out.append((pag, tipo, conf))
    return out


def segmentar(doc: OCRDocument) -> List[Segmento]:
    inicios = _filtra_continuaciones(doc, detectar_inicios(doc))
    if not inicios:
        return []
    segs: List[Segmento] = []
    for idx, (pag, tipo, conf) in enumerate(inicios):
        fin = (inicios[idx + 1][0] - 1) if idx + 1 < len(inicios) else doc.n_pages
        hojas = DOCS[tipo]["hojas"]
        if hojas:
            fin = min(fin, pag + hojas - 1)
        segs.append(Segmento(tipo, DOCS[tipo]["etiqueta"], pag, fin, round(conf, 3)))
    segs = _fusiona_contiguos(segs)
    return [_recorta_blancos(doc, s) for s in segs]


def _fusiona_contiguos(segs):
    """Une hojas seguidas del mismo tipo (los folios de un TDR, las hojas de una
    orden). Dos documentos distintos del mismo tipo quedan separados porque entre
    ellos hay hojas de otro tipo."""
    out = []
    for s in segs:
...
def resumen(segs: List[Segmento]) -> str:
    return "\n".join(f"  pág {s.pagina_ini:>2}-{s.pagina_fin:<2}  {s.etiqueta:<40} "
                     f"(conf OCR {s.confianza:.0%})" for s in segs)
